...
async def cmd_start(message: types.Message):
    ...
    if not existing_user:
        cursor.execute(
            "INSERT INTO users (user_id, username, number_balanse) VALUES (?, ?, ?)",
...
async def cmd_balance(message: types.Message):
    ...
    if balance:
        await message.answer(f"Ваш текущий баланс: {balance[0]}")
    else:
        await message.answer(
            "У вас нет счета. Для создания счета используйте команду /start"
        )
...
async def transfer_recipient(message: types.Message, state: FSMContext):
    ...
    cursor.execute(
        "SELECT user_id, balance FROM users WHERE number_balanse = ?",
    ...
    await message.answer(
        f"Перевод успешно выполнен. Остаток на счете: {sender_balance - amount}"
    )
    await state.finish()
